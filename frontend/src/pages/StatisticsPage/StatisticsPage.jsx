...
const StatisticsPage = () => {
		...
		const getStatistics = async () => {
			try {
				const response = await axios.get(
					`${apiUrl}/quizzes/${quizId}/statistics`,
				);
	...
	const averageCompletionTime = allTimeSpend.length
		? sumTimeSpend / allTimeSpend.length
		: 0;

	// Group completions by day, week, and month for bar charts
	const completionsByPeriod = quiz.results.reduce(
		(acc, result) => {
	...
	const dailyData = createChartData(
		completionsByPeriod.daily,
		"Completions per Day",
	);
	const weeklyData = createChartData(
		completionsByPeriod.weekly,
		"Completions per Week",
	);
	const monthlyData = createChartData(
		completionsByPeriod.monthly,
	...
	return (
		<div className="p-5">
			...
			{quiz.results.length ? (
				<>
					<h1>
						Average Completion Time: {averageCompletionTime.toFixed(2)} seconds
					</h1>
